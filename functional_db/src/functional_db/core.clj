...
(defn- update-attr-value
  ...
  (cond
    (util/single? attr)    (assoc attr :value #{value})
...
(defn- update-attr-in-index [index ent-id attr-name target-val operation]
  (let [colled-target-val (util/collify target-val)
        update-entry-fn (fn [indx vl] (update-entry-in-index indx ((constructs/from-eav index) ent-id attr-name vl) operation))]
...
(defn- add-entity-to-index [ent layer ind-name]
  (let [ent-id (:id ent)
        ...
        relevant-attrs (filter #((constructs/usage-pred index) %) all-attrs)
...
(defn add-entity [db ent]
  (let [[fixed-ent next-top-id] (fix-new-entity db ent)
        layer-with-updated-storage (update-in  (last (:layers db)) [:storage] storage/write-entity fixed-ent)
        add-fn (partial add-entity-to-index fixed-ent)
        new-layer (reduce add-fn layer-with-updated-storage  (constructs/indices))]
...
(defn- remove-entries-from-index [ent-id operation index attr]
  (if (= operation :db/add)
    ...
    (let  [attr-name (:name attr)
           datom-vals (util/collify (:value attr))
           paths (map #((constructs/from-eav index) ent-id attr-name %) datom-vals)]
      (reduce remove-entry-from-index index paths))))

(defn- update-index [ent-id old-attr target-val operation layer ind-name]
  (if-not ((constructs/usage-pred (get-in layer [ind-name])) old-attr)
    layer
...
(defn- put-entity [storage e-id new-attr]
  (assoc-in (storage/get-entity storage e-id) [:attrs (:name new-attr)] new-attr))

(defn- update-layer
  [layer ent-id old-attr updated-attr new-val operation]
  (let [storage (:storage layer)
        new-layer (reduce (partial update-index  ent-id old-attr new-val operation) layer (constructs/indices))]
    (assoc new-layer :storage (storage/write-entity storage (put-entity storage ent-id updated-attr)))))
...
(defn update-entity
  ...
  ([db ent-id attr-name new-val operation]
   (let [update-ts (next-ts db)
         ...
         attr (constructs/attr-at db ent-id attr-name)
...
(defn remove-entity [db ent-id]
  (let [ent (constructs/entity-at db ent-id)
        ...
        new-layer (reduce (partial remove-entity-from-index ent) no-ent-layer (constructs/indices))]
...
(defn evolution-of
  ...
  (loop [res [] ts (:curr-time db)]
    (if (= -1 ts) (reverse res)
        (let [attr (constructs/attr-at db ent-id attr-name ts)]
