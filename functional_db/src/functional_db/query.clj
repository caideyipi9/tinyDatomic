(ns functional-db.query
  (:require [functional-db.constructs :as constructs]))

"Api lib2 : 
 query language for functional-db"
...
; 取出一个clause的第一个非_的var，没有var则返回nil
...
; 将一个clause中的所有对象转化成clojure表达式，并附加var的meta信息
...
;; output:                            
;;  [#(= % %) #(= % :likes) #(= % "pizza")]
;; 元数据: {:db/variable ["?e" nil nil]}

;; [#(= % %) #(= % :name) #(= % %)]
;; 元数据: {:db/variable ["?e" nil "?name"]}

;; The following functions are used to query an index, and return the result clauses, each clause is a triplet of a path in the index and its meta
...
(defn filter-index
  ...
  (for [pred-clause predicate-clauses
        :let [[lvl1-prd lvl2-prd lvl3-prd] (apply (constructs/from-eav index) pred-clause)] ; predicates for the first and second level of the index, also keeping the path to later use its meta
...
(defn bind-variables-to-query
  ...
  [q-res index]
  (let [seq-res-path (mapcat (partial combine-path-and-meta (constructs/from-eav index)) q-res) ; seq-ing a result to hold the meta
        res-path (map #(->> %1 (partition 2) (apply (constructs/to-eav index))) seq-res-path)] ; making binding pairs
...
(defn single-index-query-plan
  ...
  [query indx db]
  (let [q-res (query-index (constructs/indx-at db indx) query)]
    (bind-variables-to-query q-res  (constructs/indx-at db indx))))
...
; The following functions are used to extract the variables from the query result, and return them in a way that is convenient for the user
...
;; 最终的用户接口函数 q，依赖于q-clauses-to-pred-clauses build-query-plan symbol-col-to-set unify 四个宏
(defmacro q [db query]
